...
internal void clear_screen(SDL_Color color) {
  SDL_SetRenderDrawColor(renderer, color.r, color.g, color.b, color.a);
  SDL_RenderClear(renderer);
}

internal void render_texture(SDL_Texture* texture, int x, int y, int w, int h) {
  SDL_Rect dst = {x, y, w, h};
  SDL_RenderCopy(renderer, texture, nullptr, &dst);
}

internal void draw_rect(SDL_Rect rect,
                        SDL_Color color,
                        bool shoud_fill = false) {
  SDL_SetRenderDrawColor(renderer, color.r, color.g, color.b, color.a);
  if (shoud_fill) {
